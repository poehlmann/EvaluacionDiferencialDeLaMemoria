...
class View extends Smarty
{
    ...
    public function __construct(Request $peticion, ACL $_acl) 
    {
        ...
        $this->_linkPlugin = array();
        $this->_template = DEFAULT_LAYOUT;
        self::$_item = null;

        $this->registerPlugin("function", "poly_bind", "print_polymer_binding");
    ...
    /*bower*/
     public function setLinkPlugin(array $link)
    {
        if(is_array($link) && count($link)){
            for($i=0; $i < count($link); $i++){
                $this->_linkPlugin[] = BASE_URL . 'views/' .  $link[$i] . '.html';
            }
        } 
        else {
            throw new Exception('Error de link plugin');
        }
    }        
...
function print_polymer_binding($params, $smarty)
    {
